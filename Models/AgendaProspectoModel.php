<?php
  ...
  class AgendaProspectoModel extends Mysql{
    ...
    public function selectAgendaProspectos(string $nomConexion){
      $sql = "SELECT id, fecha_programada, hora_programada, asunto, estatus FROM t_agenda";
      $request = $this->select_all($sql,$nomConexion);
      return $request;
    }

    public function selectAgendaProspecto(int $intIdAgenda_guardado, string $nomConexion){
    ...
    public function estatusUpdate(int $idA, int $estatus, string $nomConexion){
    ...
    public function selectNombreUsuairoCreacion(int $id, string $nomConexion){

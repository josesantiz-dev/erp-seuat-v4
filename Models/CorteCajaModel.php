<?php
	class CorteCajaModel extends Mysql
	{
        ...
        public function selectCorteActual(string $nomConexion){
        ...
        public function estatusCaja(int $idCaja, string $nomConexion){
            return $idCaja;
        }

        public function selectTotalesMetodosPago(int $id_usuario,$fecha_apertura, string $nomConexion){
        ...
        public function selectDetalleIngreso(int $idIngreso, string $nomConexion){
        ...
        public function selectMetodoPago(int $metodo, string $nomConexion){
        ...
        public function updateCorteCaja(string $folio, int $id_corte_caja, $total_entregada,int $id_user_entrega,int $id_usuario_recibe,string $comentario, string $nomConexion){
            ...
            $request = $this->update($sql,$nomConexion,array($folio,$total_entregada,$id_user_entrega,$id_usuario_recibe,$comentario));
            return $request;
        }



        public function updateStatusCaja(int $id_caja,$monto, string $nomConexion){
            $sql = "UPDATE t_estatus_caja SET estatus_caja = ?,monto_caja = ? WHERE id_caja = $id_caja";
            $request = $this->update($sql,$nomConexion,array(0,$monto));
            return $request;
        }
        public function selectIdUsuario(int $id_caja, string $nomConexion){
            $sql = "SELECT id_usuario_atiende FROM t_cajas WHERE id = $id_caja";
            $request = $this->select($sql, $nomConexion);
            return $request;
        }
        public function insertDetalleCorteCaja($cantidad_entregada,$cantidad_real_recibida,int $estatus,int $id_usuario_creacion,int $id_metodo_pago,int $id_corte_caja, string $nomConexion){
            ...
            $request = $this->insert($sql,$nomConexion,array($cantidad_entregada,$cantidad_real_recibida,$estatus,$id_usuario_creacion,$id_usuario_creacion,$id_metodo_pago,$id_corte_caja));
            return $request;
        }

        public function sigFoliocorte(string $codigo_plantel, string $nomConexion){
            $sql = "SELECT COUNT(folio) AS num_folios FROM  t_corte_caja WHERE folio LIKE '%$codigo_plantel%'";
            $request = $this->select($sql, $nomConexion);
            return $request;
        }

        public function selectPlantelCajero(int $idUsuario, string $nomConexion){
        ...
        public function insertDineroCaja($id_corte_caja,$faltante,$sobrante,$id_user,$comentario, string $nomConexion){
            ...
            $request = $this->insert($sql,$nomConexion,array($sobrante,$faltante,1,$id_user,$id_corte_caja,$comentario));
            return $request;
        }
        public function selectPlantelUsuaurio(int $idUser, string $nomConexion){

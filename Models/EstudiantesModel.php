<?php
	class EstudiantesModel extends Mysql
	{
        ...
        public function selectEstudianteInsc(int $idInscripcion, string $nomConexion){
            $this->strNomConexion = $nomConexion;
            $sql = "SELECT ins.id,per.id AS id_persona,per.nombre_persona,CONCAT(per.ap_paterno,' ',per.ap_materno) AS apellidos,
        ...
        public function selectDocumentacion(int $idInscripcion, string $nomConexion){
        ...
        public function selectDocumentacionEntregados(int $idInscripcion, string $nomConexion){
            $idInscripcion = $idInscripcion;
            $this->strNomConexion = $nomConexion;
        ...
        public function insertOriginalDocumentacion($data, string $nomConexion){
            ...
            $requestHistorial = $this->select($sqlHistorial,$this->strNomConexion);
            ...
            $requestExist = $this->select_all($sqlExist,$this->strNomConexion);
            if($requestExist){
                $id = $requestExist[0]['id'];
                $sql = "UPDATE t_documentos_estudiante SET entrego_cantidad_original = ?,estatus = ? WHERE id = $id";
                $request = $this->update($sql,$this->strNomConexion,array($cantidadOriginal,$estatus));
            }else{
                ...
                $request = $this->insert($sql,$this->strNomConexion,array(0,$cantidadOriginal,0,$estatus,$idDetalleDocumentacion,$idHistorial));
            }
            return $request;
        }
        public function insertCopiaDocumentacion($data, string $nomConexion){
            $idInscripcion = $data['idInscripcion'];
            $idDetalleDocumentacion = $data['idDetalle'];
            $tipoDocumentacion = $data['tipo'];
            $estadoCheckDocumentacion = $data['estado'];
           ...
           $sqlHistorial = "SELECT his.id FROM t_historiales AS his INNER JOIN t_inscripciones AS ins ON ins.id_historial = his.id WHERE his.inscrito = 1 AND ins.id = $idInscripcion LIMIT 1";
            ...
            if($requestExist){
                ...
                $request = $this->update($sql,$this->strNomConexion,array($cantidadCopia));
            }else{
                $sql = "INSERT INTO t_documentos_estudiante(prestamo_original,entrego_cantidad_original,entrego_cantidad_copias,estatus,id_detalle_documentos,id_historial) VALUES(?,?,?,?,?,?)";
                $request = $this->insert($sql,$this->strNomConexion,array(0,0,$cantidadCopia,0,$idDetalleDocumentacion,$idHistorial));
            }
            return $cantidadCopia;
        }
        public function insertCantidadCopiaDocumentacion($data, string $nomConexion){
            $idInscripcion = $data['idInscripcion'];
            $idDetalleDocumentacion = $data['idDetalle'];
            $tipoDocumentacion = $data['tipo'];
            $cantidadCopia = $data['cantidad'];
        ...
        public function selectEstatusDocumentacion($data, string $nomConexion){
            $idInscripcion = $data['idInscripcion'];
            $this->strNomConexion = $nomConexion;
            $sqlHistorial = "SELECT his.id FROM t_historiales AS his INNER JOIN t_inscripciones AS ins ON ins.id_historial = his.id WHERE his.inscrito = 1 AND ins.id = $idInscripcion LIMIT 1";
            $requestHistorial = $this->select($sqlHistorial,$this->strNomConexion);
            ...
            $requestEstatus = $this->select_all($sqlEstatus,$this->strNomConexion);
            return $requestEstatus;
        }
        public function insertValidacionDocumentacion($data,string $nomConexion){
            $idInscripcion = $data['idInscripcion'];
            $this->strNomConexion = $nomConexion;
            $sqlPersona = "SELECT id_personas FROM t_inscripciones WHERE id = $idInscripcion LIMIT 1";
            $requestPersona = $this->select($sqlPersona,$this->strNomConexion);
            ...
            $request = $this->update($sql,$this->strNomConexion,array(1,1));
            return $request;
        }
        public function insertValidacionDatosPersonales($data, string $nomConexion){
            ...
            $request = $this->update($sql,$this->strNomConexion,array($nombre,$appPaterno,$appMaterno,$direccion,$edad,$sexo,$CP,$colonia,$telefonoCel,$telefonofijo,$email,$estadoCivil,$ocupacion,$CURP,$fechaNacimiento,1,$localidad,1));
            return $request;
        }
        ...
        public function selectUsuarioValidacion(int $idPersonaValidacion, string $nomConexion){
        ...
        public function insertPrestamoDocumentos($idDocumentosDetalles,$idInscripcion,$comentario,$fechaDevolucion, string $nomConexion){
            ...
            $requestFolioPlantel = $this->select($sqlFolioPlantel,$this->strNomConexion);
            ...
            $requestFolioConsecutivo = $this->select($sqlFolioCosecutivo,$this->strNomConexion);
            ...
            foreach ($documentosDetalles as $key => $value) {
                ...
                $requestPrestamo = $this->insert($sqlPrestamo,$this->strNomConexion,array($nuevoFolioConsecutivo,$fechaDevolucion,$key,1,$comentario));
                if($requestPrestamo){
                    $sql = "UPDATE t_documentos_estudiante SET prestamo_original = ? WHERE id = $key";
                    $request = $this->update($sql,$this->strNomConexion,array(1));
                }else{
                }
            }
            return $request;
        }
        public function insertDevolucionDocumentos($folio,$data, string $nomConexion){
            $folioDoc = $folio;
            $datos = $data;
            $this->strNomConexion = $nomConexion;
            $resultados;
            foreach ($datos as $key => $value) {
                ...
                if($value['check'] != true){
                    ...
                    $requestDevolucion = $this->update($sqlDevolucion,$this->strNomConexion,array($comentario,1));
                    if($requestDevolucion){
                        $sql = "UPDATE t_documentos_estudiante SET prestamo_original = ? WHERE id = $idDoc";
                        $request = $this->update($sql,$this->strNomConexion,array(0));
                    }else{
                    }
                }
            }
        ...
        public function selectHistorialFoliosPrestamoDoctos($idInscripcion, string $nomConexion){
            $idInscripcion = $idInscripcion;
            $this->strNomConexion = $nomConexion;
        ...
        public function selectListaDocumentosFolio($folio, string $nomConexion){
        ...
        public function selectTutorAlumno(int $idAlumno, string $nomConexion){
            $this->strNomConexion = $nomConexion;
        ...
        public function updateTutorAlumno($data, string $nomConexion){
            ...
            $requestTutor = $this->update($sqlTutor,$this->strNomConexion,array($nombreTutor,$apPatTutor,$apMatTutor,$direccionTutor,$telCelularTutor,$telFijoTutor,$emailTutor));
            return $requestTutor;
        }
        public function selectDatosFiscales(int $idAlumno, string $nomConexion){
            $this->strNomConexion = $nomConexion;
        ...
        public function selectStatusDatosFiscales(int $idAlumno, string $nomConexion){
            $this->strNomConexion = $nomConexion;
            $sqlDatosFiscales = "SELECT per.id_datos_fiscales FROM t_personas AS per WHERE per.id = $idAlumno";
            $requestDatosFiscales = $this->select($sqlDatosFiscales,$this->strNomConexion);
            return $requestDatosFiscales;
        }
        public function insertDatosFiscales(int $idAlumno,int $CP,string $direccion,string $email,string $lugar,string $razonSocial,string $RFC,int $telefono,string $nomConexion){
            $this->strNomConexion = $nomConexion;
            $sql = "INSERT INTO t_datos_fiscales(rfc,nombre_social,direccion,lugar,cp,telefono,email) VALUES(?,?,?,?,?,?,?)";
            $request = $this->insert($sql,$this->strNomConexion,array($RFC,$razonSocial,$direccion,$lugar,$CP,$telefono,$email));
            return $request;
        }
        public function updateDatosFiscales(int $idDatosFiscales,int $CP,string $direccion,string $email,string $lugar,string $razonSocial,string $RFC,int $telefono,string $nomConexion){
            $this->strNomConexion = $nomConexion;
            $sql = "UPDATE t_datos_fiscales SET rfc = ?,nombre_social = ?,direccion = ?,lugar = ?,cp = ?,telefono = ?,email = ? WHERE id=$idDatosFiscales";
            $request = $this->update($sql,$this->strNomConexion,array($RFC,$razonSocial,$direccion,$lugar,$CP,$telefono,$email));
            return $request;
        }
        public function updateDatFiscPersona(int $idAlumno, int $idDatosFiscales, string $nomConexion){
            $this->strNomConexion = $nomConexion;
            $sql = "UPDATE t_personas SET id_datos_fiscales = ? WHERE id= $idAlumno";
            $request = $this->update($sql,$this->strNomConexion,array($idDatosFiscales));
            return $request;
        }
        //Obtener datos para la impresion de solicitud de inscripcion
        public function selectDatosImprimirCartaAut(int $idInscripcion, string $nomConexion){
            $idInscripcion = $idInscripcion;
            $this->strNomConexion = $nomConexion;

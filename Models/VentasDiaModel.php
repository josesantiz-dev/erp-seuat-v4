<?php
	class VentasDiaModel extends Mysql
	{
		...
		public function selectVentasDia($fecha,int $idUser, string $nomConexion){
        ...
        public function selectDatosAlumno(int $idAlumno, string $nomConexion){
        ...
        public function selectObservacionIngreso(int $idIngreso, string $nomConexion){
            $sql = "SELECT observaciones FROM t_ingresos WHERE id= $idIngreso LIMIT 1";
            $request = $this->select($sql, $nomConexion);
            return $request;
        }

        public function selectDetallesVenta(int $idIngreso, string $nomConexion){
        ...
        public function selectAllVentasDia(int $idUsuario, $fecha, string $nomConexion){
            $sql = "SELECT i.id, i.folio,i.fecha,i.total,i.id_persona FROM t_ingresos AS i
